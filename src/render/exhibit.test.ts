...
import { focusAt, focusSpans, type FocusTiming } from "./exhibit.ts";
...
test("a span lasts as long as the sentence that opened it", () => {
  const spans = focusSpans(scene([{ element: 1, from: 100, length: 90 }]), REGIONS);
  assert.deepEqual(
    spans.map((span) => [span.region.name, span.from, span.until]),
    [["b", 100, 190]],
  );
});

test("only regions the picture actually has become spans", () => {
  ...
  const spans = focusSpans(scene([{ element: 9, from: 10, length: 10 }]), REGIONS);
  assert.deepEqual(spans, []);
});

test("spans come out in frame order however the anchors were listed", () => {
  const spans = focusSpans(
    scene([
  ...
  assert.deepEqual(
    spans.map((span) => span.region.name),
    ["a", "c"],
  );
});

test("nothing is veiled before the first sentence, or long after the last", () => {
  const spans = focusSpans(scene([{ element: 0, from: 100, length: 60 }]), REGIONS);
  assert.equal(focusAt(spans, 99, TIMING), undefined);
  assert.equal(focusAt(spans, 160 + TIMING.fall + 1, TIMING), undefined);
});

test("the veil rises, holds for the whole sentence, and settles", () => {
  const spans = focusSpans(scene([{ element: 0, from: 100, length: 60 }]), REGIONS);
  const veil = (frame: number): number => focusAt(spans, frame, TIMING)?.veil ?? 0;
...
test("a sentence shorter than the rise still reaches full strength", () => {
  const spans = focusSpans(scene([{ element: 0, from: 100, length: 4 }]), REGIONS);
  assert.equal(focusAt(spans, 104, TIMING)?.veil, 1);
});
...
test("two sentences in a row hand over without the veil dropping", () => {
  ...
  for (let frame = 110; frame <= 220; frame += 1) {
    assert.equal(
      focusAt(spans, frame, TIMING)?.veil,
      1,
...
test("a pause between two chart sentences does not strobe", () => {
  ...
  for (let frame = 110; frame <= 190; frame += 1) {
    assert.equal(focusAt(spans, frame, TIMING)?.veil, 1, `dipped at frame ${frame}`);
  }
});
...
test("two sentences far apart release in between, because the narration left", () => {
  ...
  assert.equal(focusAt(spans, 200, TIMING), undefined);
  assert.equal(focusAt(spans, 410, TIMING)?.veil, 1);
});
...
test("the opening travels to the next region rather than cutting to it", () => {
  ...
  assert.equal(focusAt(spans, 159, TIMING)?.region.left, 0.0);
  // Halfway through the handover, halfway between the two.
  assert.equal(focusAt(spans, 165, TIMING)?.region.left, 0.4);
  assert.equal(focusAt(spans, 170, TIMING)?.region.left, 0.8);
  assert.equal(focusAt(spans, 200, TIMING)?.region.left, 0.8);
  // It carries the name of where it is going, so a diagnostic never says it is somewhere it is not.
  assert.equal(focusAt(spans, 165, TIMING)?.region.name, "c");
});

test("the first opening does not travel from nowhere", () => {
  const spans = focusSpans(scene([{ element: 1, from: 100, length: 60 }]), REGIONS);
  assert.equal(focusAt(spans, 101, TIMING)?.region.left, 0.4);
});

test("a picture nobody talks about is never veiled", () => {
  assert.deepEqual(focusSpans(scene([]), REGIONS), []);
  assert.equal(focusAt([], 500, TIMING), undefined);
});
